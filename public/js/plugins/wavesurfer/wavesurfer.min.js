...
var WaveSurfer = {
    ...
    mark: function (t) {
        ...
        }), this.fireEvent("marked", r), r.update(i)
    },
    ...
    drawBuffer: function () {
        var t = this.drawer.getPixels(this.backend.getDuration()),
            e = this.backend.getPeaks(t),
            i = this.backend.getMaxPeak(e);
            this.fireEvent('buffer', e);
        this.drawer.drawPeaks(e, i), this.drawer.progress(this.backend.getPlayedPercents()), this.redrawMarks()
    },
    ...
    bindDragNDrop: function (t) {
        ...
        i.addEventListener("progress", function (t) {
            ...
            var dv = window.jDataView(t.target.result);

            if (dv.getString(3, dv.byteLength - 128) == 'TAG') {
                var id3Data = {};
              id3Data.title = dv.getString(30, dv.tell());
               id3Data.artist = dv.getString(30, dv.tell());
               id3Data.album = dv.getString(30, dv.tell());
               id3Data.year = dv.getString(4, dv.tell());

               e.fireEvent('id3v1', id3Data);
            } else {
              // no ID3v1 data found.
            }
        ...
        t.addEventListener("drop", function (s) {
            ...
            a ? (i.readAsArrayBuffer(a), e.empty()) : e.fireEvent("error", "Not a file")
    ...
    bindMarks: function () {
        ...
        this.backend.on("play", function () {
            ...
            Object.keys(t.markers).forEach(function (r) {
                var s = t.markers[r];
                s.played || i >= s.position && s.position > e && (s.played = !0, t.fireEvent("mark", s), s.fireEvent("reached"))
            }), e = i
        })
    },
    empty: function () {
        this.pause(), this.backend.loadEmpty(), this.drawer.drawPeaks({
...
WaveSurfer.Mark = {
    ...
    init: function (t) {
        if (!window.AudioContext && !window.webkitAudioContext) throw Error("wavesurfer.js: your browser doesn't support WebAudio");
        this.params = t, this.ac = t.audioContext || this.getAudioContext(), this.createVolumeNode(), this.createScriptNode()
    },
    ...
    refreshBufferSource: function () {
        this.source && this.source.disconnect(), this.source = this.ac.createBufferSource(), this.buffer && (this.source.buffer = this.buffer), this.source.connect(this.gainNode)
    },
    ...
    pause: function () {
        this.lastPause = this.lastStart + (this.ac.currentTime - this.startTime), this.paused = !0, this.source && (this.source.stop ? this.source.stop(0) : this.source.noteOff(0), this.source.disconnect(), this.source = null), this.fireEvent("pause")
    },
    getPeaks: function (t, e) {
        e = e || 128;
        for (var i = this.buffer, r = i.length / t, s = new Float32Array(t), a = 0; i.numberOfChannels > a; a++)
            for (var n = i.getChannelData(a), o = 0; t > o; o++) {
                for (var h = -1 / 0, u = ~~ (o * r), d = (o + 1) * r, c = u; d > c; c += e) {
                    var l = n[c];
                    l > h ? h = l : -l > h && (h = -l)
                }
                a > 0 ? s[o] += h : s[o] = h
    ...
    getMaxPeak: function (t) {
        return this.params.normalize ? WaveSurfer.util.max(t) : 1 * this.buffer.numberOfChannels
    },
    ...
    init: function (t) {
        if (this.container = "string" == typeof t.container ? document.querySelector(t.container) : t.container, !this.container) throw Error("wavesurfer.js: container element not found");
    ...
    drawPeaks: function (t, e) {
        this.setWidth(t.length), this.resetScroll(), this.drawWave(t, e)
    },
    style: function (t, e) {
    ...
    getPixels: function (t) {
        var e = Math.ceil(t * this.params.minPxPerSec);
        return this.params.fillParent && (e = Math.max(this.containerWidth, e)), e * this.pixelRatio
    },
    getWidth: function () {
        return this.width
    },
    setWidth: function (t) {
        this.width = t, this.scrollWidth = ~~ (this.width / this.pixelRatio), this.containerWidth = this.container.clientWidth, this.params.fillParent || this.params.scrollParent || this.style(this.wrapper, {
    ...
    progress: function (t) {
        ...
        (this.lastPos > i || i - this.lastPos >= e) && (this.lastPos = i, this.params.scrollParent && this.recenterOnPosition(~~(this.scrollWidth * t)), this.updateProgress(t))
    },
    ...
    drawWave: function (t, e) {
        for (var i = 0; this.width > i; i++) {
            var r = e > 0 ? Math.round(t[i] * (this.height / e)) : 1,
                s = Math.round((this.height - r) / 2);
            
            var grad = this.waveCc.createLinearGradient(0,0,0,170);
            grad.addColorStop(0,"ef6500");
            grad.addColorStop(1,"ff3a68");

            this.waveCc.fillStyle = this.params.waveColor, this.waveCc.fillRect(i, s, 1, r), this.progressCc.fillStyle = grad, this.progressCc.fillRect(i, s, 1, r)
        }
    },
    ...
    drawMark: function (t) {
        ...
        var e = Math.round(t.percentage * this.width - t.width / 2);
...
}), WaveSurfer.Drawer.SVG = Object.create(WaveSurfer.Drawer), WaveSurfer.util.extend(WaveSurfer.Drawer.SVG, {
    attr: function (t, e) {
        Object.keys(e).forEach(function (i) {
            var r = "href" == i ? "http://www.w3.org/1999/xlink" : null;
            t.setAttributeNS(r, i, e[i])
        })
    },
    node: function (t, e) {
        var i = document.createElementNS("http://www.w3.org/2000/svg", t);
        return e && this.attr(i, e), i
    },
    createElements: function () {
        var t = this.node("svg"),
            e = t.appendChild(this.node("defs")),
            i = WaveSurfer.util.getId(),
            r = e.appendChild(this.node("path", {
                id: i
            })),
            s = WaveSurfer.util.getId(),
            a = e.appendChild(this.node("clipPath", {
                id: s
            })).appendChild(this.node("rect", {
                width: 0,
                height: this.height
            }));
        t.appendChild(this.node("use", {
        ...
        var n = t.appendChild(this.node("rect", {
        ...
        this.style(t, {
            userSelect: "none",
            webkitUserSelect: "none"
        }), this.wrapper.appendChild(t), this.svg = t, this.wavePath = r, this.progressNode = a, this.cursor = n
    },
    updateWidth: function () {
        this.params.fillParent && this.attr(this.svg, {
            viewBox: [.5, .5, this.width, this.height].join(" ")
        }), (this.params.scrollParent || !this.params.fillParent) && this.attr(this.svg, {
    ...
    drawWave: function (t, e) {
        var i = this.height,
            r = [];
        if (0 == e) r.push("M", 0, Math.round(i / 2), "l", this.width, 0);
        else
            for (var s = i / e, a = 0; this.width > a; a++) {
                var n = Math.round(t[a] * s),
                    o = Math.round((i - n) / 2);
                r.push("M", a, o, "l", 0, n)
            }
        this.wavePath.setAttribute("d", r.join(" "))
    },
    updateProgress: function () {
        this.progressNode.setAttribute("width", this.lastPos), this.cursor.setAttribute("x", Math.min(this.lastPos - ~~(this.params.cursorWidth / 2), this.width - this.params.cursorWidth))
    },
    addMark: function (t) {
        var e = document.getElementById(t.id);
        if (e) var i = e.querySelector("title");
        else e = this.node("rect"), e.setAttribute("id", t.id), this.svg.appendChild(e), i = this.node("title"), e.appendChild(i);
        this.attr(e, {
            fill: t.color,
            width: t.width,
            height: this.height,
            x: Math.round(t.percentage * this.width)
        }), i.textContent = t.getTitle()
    },
    removeMark: function (t) {
        var e = document.getElementById(t.id);
